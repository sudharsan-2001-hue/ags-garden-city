  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.success && data.user) {
        ...
        setErrorMessage(
          data.message ||
          'Invalid email/phone or password. If you are a new customer, please register.'
        );
      }
    } catch (err) {
      console.warn('Login offline fallback triggered:', err);
      setIsLoading(false);
      const isEmail = identifier.includes('@');
      const fallbackUser = {
        name: isEmail ? identifier.split('@')[0].toUpperCase() : 'VIP Customer',
        email: isEmail ? identifier : `${identifier}@agsgarden.com`,
        phone: !isEmail ? identifier : '+91 98401 23456',
        role: 'buyer'
      };
      if (rememberMe) {
        localStorage.setItem('agsgarden_customer', JSON.stringify(fallbackUser));
      }
      if (onLoginSuccess) {
